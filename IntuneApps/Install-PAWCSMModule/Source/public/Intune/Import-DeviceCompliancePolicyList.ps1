function Import-DeviceCompliancePolicyList {
    <#
    .SYNOPSIS
    Imports a list of Device Compliance Policies
    .PARAMETER JSONFileList
    The list of json files that contain the Device Compliance Policies to import
    .EXAMPLE
    $jsonFileList = Get-ChildItems -Path .\JSON\DeviceCompliancePolicy\ | Select Fullname -Expand FullName
    Import-DeviceCompliancePolicyList -JSONFileList $jsonFileList

    In this example, the list of json files is created with the Get-ChildItem cmdlet and the
    results are passed directly to the Import-DeviceCompliancePolicyList for processing
    #>
    [cmdletbinding()]
    param (
        [Parameter(Mandatory)]
    ...
    Write-Log -Message "`nDevice Compliance Policy:" -WriteHost White
    foreach ($JSONFile in $JSONFileList) {

        $deviceCompliancePolicyJSON = Get-JSONContent -JSONFile $JSONFile

        if ( Test-DeviceCompliancePolicy -DisplayName $deviceCompliancePolicyJSON.DisplayName ) {
            Write-Log -Message "  Existing: $($deviceCompliancePolicyJSON.DisplayName)" -WriteHost Yellow
        }
        else {
            Write-Log -Message "  Creating: $($deviceCompliancePolicyJSON.DisplayName)" -WriteHost Green

            $CompliancePolicy = Set-DeviceCompliancePolicy -JSON $deviceCompliancePolicyJSON

            $ComplianceAssignments = @()

            foreach ($AADGroup in $deviceCompliancePolicyJSON.assignments.target ) {
            ...
            Set-DeviceCompliancePolicyAssignment -ComplianceAssignments $ComplianceAssignments -CompliancePolicyId $CompliancePolicy.Id
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBLGNI2dBHbd6v3
# qkGXuegJ8/edYLsphw5S9MgSKofrB6CCDYUwggYDMIID66ADAgECAhMzAAACzfNk
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIB2Q
# cgpqhVS/tsmbfKPG3AWW/6iLrn/J48UtAAJx8JFwMEIGCisGAQQBgjcCAQwxNDAy
# oBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5j
# b20wDQYJKoZIhvcNAQEBBQAEggEAEUz5ahofiLqSlpYBgji0StZIn9+7Dkko+UKZ
# o/dhu2VgBC7Wwijwog31HEcsuNKIGWP5ZvPvh13Hl7wRWm9ntWpLmngR3l5DT+6p
# M3MXsD/WKGXy60A61Giws/5HgkPewZ9jKSQJVyl7TZQttyvGS+m73mKgPpXZj+uj
# 66IVwR4MUAmRShFdqA1Fy4O5SHMg5k1y6NAJJKFUal5ffu/4s4rnWB0BnI6wC1nH
# 2N0+u6cJqiG6nm01AjnuhApnXXV4/8/XxPtMr9vKKPce9SV6ZOdoQpfZO9vUBoQC
# oF+kOCEJAz/jxoK6TulSt0q+vBCa+fcHXfP0WWaA4U+L3GSyAKGCFygwghckBgor
...
# WQoDATAxMA0GCWCGSAFlAwQCAQUABCBYmJKOe1/HLlPI2xf/3Jz/TOdVLjvcSRfp
# rYJ2EQt0MQIGZBsBhr5tGBIyMDIzMDQxMjIxMTAxNy43NlowBIACAfSggdikgdUw
...
# CSqGSIb3DQEBBQUAAgUA5+Es2zAiGA8yMDIzMDQxMjIxMTI1OVoYDzIwMjMwNDEz
