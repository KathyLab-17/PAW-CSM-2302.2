function Import-NewStoreAppsList {
    [cmdletbinding()]
    param (
        [Parameter(Mandatory)]
        [string[]] $JSONFileList
    ...
    Write-Log -Message "Creating and assignning New Store Apps in Intune" -WriteHost White
    foreach ($JSONFile in $JSONFileList) {
        ...
        foreach ($app in $appList) {
            ...
            if (-not (Test-NewStoreApp -Name $app.packageName -PackageId $app.AppId)) {
                
                Write-Log -Message "`Creating $($app.packageName)" -WriteHost Green

                # Update Tag Names
                if ( -not [string]::IsNullOrWhiteSpace($app.roleScopeTagIds) ) {
                    $roleScopeTagIds = @()
                    foreach ($id in $app.roleScopeTagIds) {
                        $tagDisplayName = ConvertTo-TagDisplayName -Id $id
                        $intuneTag = Get-IntuneRoleScopeTag -displayName $tagDisplayName
                        $roleScopeTagIds += $intuneTag.Id
                    }
                    $app.roleScopeTagIds = $roleScopeTagIds
                }

                Set-NewStoreApp -AppName $app.packageName -PackageId $app.AppId -roleScopeTagIds $app.roleScopeTagIds -installExperience $app.installExperience | Out-Null

                $Params = @{
                    PackageName    = $app.packageName
                    GroupToAdd     = ConvertTo-AADDisplayName -Id $app.groupIdToAdd
                    AssignmentType = $app.assignmentType
                    AppType        = $app.appType
                }
        
                Write-Log -Message "`Waiting 15 seconds before start assigning the app to avoid assignment errors." -WriteHost White
                Start-Sleep -Seconds 15

                # \WindowsPowerShell\Modules\PAWCSM\private\Intune
                Write-Log -Message "`Assigning $($app.packageName)" -WriteHost Green
                Set-PackageAADGroupMembership `
                    -PackageName $Params.PackageName `
                    -GroupToAdd $Params.GroupToAdd `
                    -AssignmentType $Params.AssignmentType `
                    -AppType $Params.AppType

            }
            else {
                Write-Log -Message "$($app.packageName) already exists" -WriteHost Yellow
            }
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD77aKWSXQEERQV
# arVgAA7THDzOd3JpdrzhzKYbPXfhvqCCDYUwggYDMIID66ADAgECAhMzAAACzfNk
...
# tTvBA1CGhjs2tumT4gF2y+XOoj6Z9WA4ICNnfXiHMEIGCisGAQQBgjcCAQwxNDAy
# oBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5j
# b20wDQYJKoZIhvcNAQEBBQAEggEAchYfUcBuwazpBl0x9lzaPHhPxGvvUPvCmfrX
# JkywX85C9BnGtcAQh+2kx7mzJ5PgwhjwyrY0l4bIdmF5jUG6sL+/SPkablJz/zE4
# w+jNxQVtf7+Isz7Rgo9mb9x/GabuH8LSaZf/N1Gcg2HnlhX09/aH84xwkxbaKij+
