function Import-AutoPilotPolicyList {
    <#
    .SYNOPSIS
    Imports a list of Intune AutoPilot profiles
    .PARAMETER JSONFileList
    The list of json files that contain the AutoPilot Profiles to import
    .EXAMPLE
    $jsonFileList = Get-ChildItems -Path .\JSON\AutoPilot\ | Select Fullname -Expand FullName
    Import-AutoPilotPolicyList -JSONFileList $jsonFileList

    In this example, the list of json files is created with the Get-ChildItem cmdlet and the
    results are passed directly to the Import-AutoPilotPolicyList for processing
    #>
    [cmdletbinding()]
    param (
        [Parameter(Mandatory)]
    ...
    Write-Log -Message "`nAutoPilot Policy:" -WriteHost White
    foreach ($JSONFile in $JSONFileList) {

        $autopilotProfileJSON = Get-JSONContent -JSONFile $JSONFile

        if ( Test-AutoPilotPolicy -DisplayName $autopilotProfileJSON.displayName ) {
            Write-Log -Message "  Existing: $($autopilotProfileJSON.DisplayName)" -WriteHost Yellow
        }
        else {
            Write-Log -Message "  Creating: $($autopilotProfileJSON.DisplayName)" -WriteHost Green

            $AutoPilotProfile = Set-AutoPilotPolicy -JSON $autopilotProfileJSON

            foreach ($aadGroup in $autopilotProfileJSON.assignments.target ) {
                ...
                $null = Set-AutoPilotPolicyAssignment -AutoPilotProfileId $AutoPilotProfile.Id -TargetGroupId $TargetGroup.id -Assignment $aadGroup."@OData.type"
            }
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAkHh7pV4lWTpe/
# JcLWgF5cUOc1TiCvCQVrEppUyLMVEKCCDYUwggYDMIID66ADAgECAhMzAAACzfNk
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEICsK
# iNZiYWLp07GFtzLLetstx0Pm+czF/KZyGqHWyeqmMEIGCisGAQQBgjcCAQwxNDAy
# oBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5j
# b20wDQYJKoZIhvcNAQEBBQAEggEAtJeIsl53V2P4B/yQfdkjbyhTKnZrYz8s0kP9
# W87SHPA7dwMBM9dUw4RE5eyhq2+ncR0B+sXvAz70rbSktIsrnFsvA9F2bUrBvxaW
# +RK+1gpquM0hxV8ysnT0gMwQSc8mmsdBNb2QL4YVKqn6lvQnbmnD7fmBU+1gKHMy
# eZjiKqXYixHjbuj2YjutadxjIKdDYsrck971l9FsOILBziEFvq2MILbXEj+c5NfI
# gpT1qsE8QYiyh98h7qph3YVWsuIlE2+M5DHDBvhl8jzoFx/K+IQZIRD9wNN/wOVn
