function Remove-DeviceCompliancePolicyList {
    <#
    .SYNOPSIS
    Removes a list of Device Compliance Policies
    .PARAMETER JSONFileList
    The list of json files that contain the Device Compliance Policies to remove
    .EXAMPLE
    $jsonFileList = Get-ChildItems -Path .\JSON\DeviceCompliancePolicy\ | Select Fullname -Expand FullName
    Remove-DeviceCompliancePolicyList -JSONFileList $jsonFileList

    In this example, the list of json files is created with the Get-ChildItem cmdlet and the
    results are passed directly to the Remove-DeviceCompliancePolicyList for processing
    #>
    [cmdletbinding()]
    param (
        [Parameter(Mandatory)]
    ...
    Write-Host "`nDevice Compliance Policy:"

    foreach ($JSONFile in $JSONFileList) {

        $deviceCompliancePolicyJSON = Get-JSONContent -JSONFile $JSONFile -ExcludeProperty 'id','createdDateTime','lastModifiedDateTime','scheduledActionConfigurations@odata.context'

        if ( Test-DeviceCompliancePolicy -DisplayName $deviceCompliancePolicyJSON.DisplayName ) {
            Write-Host '  Removing:' $deviceCompliancePolicyJSON.DisplayName -f Red

            $compliancePolicy = Get-DeviceCompliancePolicy -DisplayName $deviceCompliancePolicyJSON.DisplayName

            $apiVersion = 'beta'
            $resource = "deviceManagement/deviceCompliancePolicies/$($compliancePolicy.id)"
            ...
            Write-Host '  Not Found:' $deviceCompliancePolicyJSON.DisplayName -f Yellow
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAu0DSbcemU/pup
# 29G4ExUr5lDIERYHDcWIgGf62XWGr6CCDYUwggYDMIID66ADAgECAhMzAAACzfNk
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIESs
# QvvhjCrzPyCTupiMTc9Z4WNu7ZjqagA4iMeXjoZ6MEIGCisGAQQBgjcCAQwxNDAy
# oBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5j
# b20wDQYJKoZIhvcNAQEBBQAEggEAmwHNiNXgpIwUEsUmkBPMn+3FPg95w7VebRi8
# DsNu3c5whybVGVqD5RymAm4xXNjnZCP5HNICQC/0haKr+q/5IKx/+tIyxKYxyVWD
# kmhfw0TMyznFYcCMSnDdLiJ/Za3MiqzFSvpPCSHBm9USw/tnAr3oTt1rYoHFa/VM
# /2W9YiqzKanOLOC3COVXvh3G+f3467np5d6pJVMX735uGyJeCIADaTrn/Kb+83QT
# cTVLCyr1xTYphW3v0Q7fEQQpMeJbp6Rzl9RSNIdeeTF27axzXSMDE/bUyJsQnNvh
# vEKj8guhlB9gjEwzAUeCDLxnZK4adpJ5jI3y26Y7fgJ4LFMb6KGCFvwwghb4Bgor
...
# WQoDATAxMA0GCWCGSAFlAwQCAQUABCD4jDv2th2Sg6Tk9gvxoQNFnSCbhgf7aolP
# fF9v8UMyrQIGZBNQb4iqGBIyMDIzMDQxMjIxMTAxOS43OFowBIACAfSggdCkgc0w
...
# CRABBDAvBgkqhkiG9w0BCQQxIgQgvvUY3rdCSM9qDuSe779+w6RlxXqo37q0PDRS
# +6llQZYwgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9BCD9Di1HPrcSJGPgr7X3
...
# bOmuarihPn8clG/51WIjuk4lMA0GCSqGSIb3DQEBCwUABIICALUpyDVS8c4nisH3
# u7DwzBPnPmEbyo4WOC5Gi/UNcMDXqZ8gGXAim4Vkx7eY0+ubi3i2FLYrmu0mC1w6
# 4JXyGfvU5Xy8GYSeZaNNxk581wi7JsNTkpqpQnqfG7rz4bctmHVJhs+KUnBtoqea
# 6DNJ5yX8oquhpUmXzZUmte12QNGvYSCiXPMyK0LTlO3UJ/KtVxBSqK56+/oU1W5E
# Hw0yNgy6iJ0hk9aIuSnn3sZdND+tcwZUA7ZrD3M4iv/q3R5/ZOL1mC4Poj61mgIV
# azv+4ELyvz8MIqkrdeYvwEtqr6XMp2wx8jwE2kR0Id9XCawvYjJSCP3ymKLxoidY
# kPds1jsSGUjnV4Ji2RYPx3chePMZRYIbuK3DInEKf+6/AvqIy4mwj9fDYWdtEbHd
# sPaVNyAUu8h++ighp/eomYmtQ7gBYu2APGpzqVnV+G1bbLcvAl/d7m+LVCVd/qme
# G9eInodNrbEpm03EtqUvXEEnETsgO7YJVOTPy6FwSXVyt5jB9ToO2E+DU6VZbvzt
# bLZNxUwFqsKW9c0AxZJMcrjl06N8Tn2KSo/tSM5vnmItPbYy6RPqJ+qb1xO+KA0l
# swGSA8fUiWpo2xIvV8Bc+1ql7HR8vvrTMzIxNt6uun0GZ1ZX1xE0oiAXPoxB/4oA
# l1i1Z5LNOmUPAxQMjf7gQg8bGmkx
